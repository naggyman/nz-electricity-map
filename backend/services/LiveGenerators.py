...
nodeSkipList = ["KAW0112 DLS0", "KAW0113 DLS1"]

class LiveGenerators:
    def __init__(self, generatorDescriptions: GeneratorDescriptions, realTimeDispatch: RealTimeDispatch, outages: Outages) -> None:
    ...
    def getLiveGeneratorOutput(self):
        ...
        for node in self.realTimeDispatch.unclaimedGeneration():
            if len(node['PointOfConnectionCode'][7:]) > 0 and node['PointOfConnectionCode'] not in nodeSkipList:
